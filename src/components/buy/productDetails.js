...
const ProductDetails = (props) => {
      ...
      .then((res) => {
        if (res.data.success) {
          ...
          setFavorit(res.data.favorit)(
            res.data.success.images.length > 0
              ? setBgImage(res.data.success.images[0])
              : null
          );
        }
      })
  ...
  const deactivateHandler = async (id) => {
    await axios
      .post(
        `${process.env.REACT_APP_DB_HOST}/api/products/inactiveproduct`,
        { data: { id } },
        {
          headers: {
            "x-auth-token": localStorage.getItem("c2c-token"),
  ...
  const soldHandler = (id) => {
    axios
      .post(
        `${process.env.REACT_APP_DB_HOST}/api/products/soldproduct`,
        { data: { id } },
        {
          headers: {
            "x-auth-token": localStorage.getItem("c2c-token"),
  ...
  const activateHandler = (id) =>
    axios
      .post(
        `${process.env.REACT_APP_DB_HOST}/api/products/activateproduct`,
        { data: { id } },
        {
          headers: {
            "x-auth-token": localStorage.getItem("c2c-token"),
  ...
  const deleteHandler = (id) =>
    axios
      .post(
        `${process.env.REACT_APP_DB_HOST}/api/products/deleteproduct`,
        { data: { id } },
        {
          headers: {
            "x-auth-token": localStorage.getItem("c2c-token"),
  ...
  const favoriteHandler = async (id) => {
    if (localStorage.getItem("c2c-token")) {
      ...
      let response = await POST(
        `${process.env.REACT_APP_DB_HOST}/api/account/setfavorities`,
        id,
        config
      );
