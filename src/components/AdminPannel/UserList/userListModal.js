...
function UserListModal({ closeHandler, userId, getUserList }) {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_DB_HOST}/api/admin/userdetails/${userId}`, {
        headers: {
        ...
        if (res.data.data) {
          ...
          setAdmin(res.data.data.admin)(
            res.data.data.profileImage
              ? setBgImage(res.data.data.profileImage)
              : null
          );
        }
      })
    ...
    <div className="adminRedBox">
      ...
      {recordUpdated ? (
        <p className="text-success">
          You have successfully updated the user Access Level.
        </p>
      ) : null}
      {showUpdate ? (
        <button
