...
function ProductList() {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_DB_HOST}/api/admin/activeproducts`, {
        headers: {
  ...
  const searchData = (searchText, searchCategory) => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_DB_HOST}/api/admin/searchproduct`,
        { id: searchText, type: filterType },
        {
          headers: {
            "x-auth-token": localStorage.getItem("c2c-token"),
            "Content-Type": "application/json",
  ...
  const getProducts = (productType) => {
    axios
      .get(`${process.env.REACT_APP_DB_HOST}/api/admin/${productType}`, {
        headers: {
          "x-auth-token": localStorage.getItem("c2c-token"),
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className="mt-5">
      ...
      {products.length > 0 ? null : (
        <div>
          <h3>Currently there are no Products to show</h3>
        </div>
      )}
    </div>
  );
