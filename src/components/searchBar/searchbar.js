...
export default function SearchBar(props) {
  ...
  const changeHandler = (e) => {
    ...
    if (text.length > 0) {
      ...
      suggestionArray = items
        .sort()
        .filter((v) =>
          v.toLowerCase().slice(0, length).includes(text.toLowerCase(), 0)
        )
        .slice(0, 10);
  ...
  const submitHandler = (e) => {
    ...
    if (searchText.length < 1 && searchCategory === props.category) return;
      ...
      props.history.push({
        pathname: `/categories/${
          Categories.filter((key) => key.id === searchCategory)[0].linkName
        }`,
      ...
      <form onSubmit={submitHandler} noValidate="novalidate">
        <div className="row ">
            ...
            <ul style={{ border: "none" }}>
              {suggestions.length > 0
                ? suggestions.map((item, index) => (
                    <li key={index} onClick={() => suggestionSelector(item)}>
                      {item}
                    </li>
                  ))
                : null}
          ...
          <div className="col-lg-3 col-md-3 col-sm-12 p-2">
            <button type="submit" className="myBlueButton-sm wrn-btn">
              Search
            </button>
          </div>
        </div>
