...
const NewMessage = (props) => {
  ...
  const submitHandler = async () => {
    ...
    let response = await POST(
      `${process.env.REACT_APP_DB_HOST}/api/messages/sendmessage`,
      messageData,
      config
    );
  ...
  return (
    <div ref={ref}>
      <div
        className="myIcons fa fa-envelope-o"
        variant="secondary"
        onClick={handleClick}
      ></div>
        ...
        <Popover id="popover-contained">
          {auth ? (
            <div className="s-m-b-title mb-3">
              <Popover.Title as="h3" className="bestRedColor">
            ...
            <div className="s-m-title">
              ...
              <Popover.Content>
                <small className="mt-3 myText">
                  You are not logged in please{" "}
                  <Link onClick={openShowSignin}>Signin</Link>
                </small>
              </Popover.Content>
            </div>
          )}
