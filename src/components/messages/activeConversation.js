...
const ActiveConversation = (props) => {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      let res = await GET(
        `${process.env.REACT_APP_DB_HOST}/api/messages/getconversation/${conversationId}`
      );
      if (res.data.data) {
  ...
  const submitHandler = async (e) => {
    ...
    if (message === "") return;
      ...
      let res = await POST(
        `${process.env.REACT_APP_DB_HOST}/api/messages/updateconversation`,
        messageData,
        config
      );
  ...
  return (
    <div className="message-wrapper">
      ...
      <div className="p-id justify-content-around">
        <p>
          Product Title :{" "}
          {prevMessages.productId ? prevMessages.productId.title : null}
        </p>
        <p>
          Product ID :
          {prevMessages.productId ? prevMessages.productId._id : null}
        </p>
      </div>
      <div className="message-box">
        <ScrollToBottom className={ROOT_CSS}>
          {prevMessages.productId
